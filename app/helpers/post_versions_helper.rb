module PostVersionsHelper
  def post_version_diff(post_version)
    ...
    diff[:added_tags].each do |tag|
      ...
      html << prefix + link_to(wordbreakify(tag), booru_posts_path(Booru.current.slug, :tags => tag)) + '</ins>'
      html << " "
    end
    diff[:removed_tags].each do |tag|
      prefix = diff[:obsolete_removed_tags].include?(tag) ? '-<del class="obsolete">' : '<del>-'
      html << prefix + link_to(wordbreakify(tag), booru_posts_path(Booru.current.slug, :tags => tag)) + '</del>'
      html << " "
    end
    diff[:unchanged_tags].each do |tag|
      html << '<span>' + link_to(wordbreakify(tag), booru_posts_path(Booru.current.slug, :tags => tag)) + '</span>'
      html << " "
    end
